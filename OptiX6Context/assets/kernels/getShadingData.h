...
__constant__ __device__ uint **textures;

using namespace rfw;
using namespace glm;

template <uint S> __device__ static inline float char2flt(const unsigned int value)
{
...
struct ShadingData
{
	...
	uvec4 parameters;
	__device__ inline float getMetallic() const { return char2flt<0>(parameters.x); }
	__device__ inline float getSubsurface() const { return char2flt<8>(parameters.x); }
	__device__ inline float getSpecular() const { return char2flt<16>(parameters.x); }
	__device__ inline float getRoughness() const { return max(0.001f, char2flt<24>(parameters.x)); }
	__device__ inline float getSpecularTint() const { return char2flt<0>(parameters.y); }
	__device__ inline float getAnisotropic() const { return char2flt<8>(parameters.y); }
	__device__ inline float getSheen() const { return char2flt<16>(parameters.y); }
	__device__ inline float getSheenTint() const { return char2flt<24>(parameters.y); }
	__device__ inline float getClearCoat() const { return char2flt<0>(parameters.z); }
	__device__ inline float getClearCoatingGloss() const { return char2flt<8>(parameters.z); }
	__device__ inline float getTransmission() const { return char2flt<16>(parameters.z); }
	__device__ inline float getEta() const { return char2flt<24>(parameters.z); }
	__device__ inline float getCustom1() const { return char2flt<0>(parameters.w); }
	__device__ inline float getCustom2() const { return char2flt<8>(parameters.w); }
	__device__ inline float getCustom3() const { return char2flt<16>(parameters.w); }
	__device__ inline float getCustom4() const { return char2flt<24>(parameters.w); }

	__device__ inline bool isEmissive() const { return any(glm::greaterThan(color, vec3(1.0f))); }
};

inline __device__ glm::vec4 __uchar4_to_float4(const uint v4)
{
	const float r = 1.0f / 256.0f;
	return glm::vec4((float)(v4 & 255u) * r, (float)((v4 >> 8u) & 255u) * r, (float)((v4 >> 16u) & 255u) * r,
					 (float)(v4 >> 24u) * r);
}

inline __device__ glm::vec4 FetchTexel(const glm::vec2 texCoord, const int o, const int w, const int h,
									   const TexelStorage storage = RGBA32)
{
	...
	if (storage == RGBA32)
		...
		p2 = __uchar4_to_float4(uintTextures[o + iu + iv1 * w]),
		p3 = __uchar4_to_float4(uintTextures[o + iu1 + iv1 * w]);
	else if (storage == RGBA128)
	{
		p0 = floatTextures[o + iu + iv * w], p1 = floatTextures[o + iu1 + iv * w], p2 = floatTextures[o + iu + iv1 * w],
		p3 = floatTextures[o + iu1 + iv1 * w];
	}
...
inline __device__ glm::vec4 FetchTexelTrilinear(const float lambda, const glm::vec2 texCoord, const int offset,
...
inline static __device__ ShadingData getShadingData(const glm::vec3 D, const float u, const float v,
													const float coneWidth, const rfw::DeviceTriangle &triangle,
													const int instanceIndex, glm::vec3 &N, glm::vec3 &iN, glm::vec3 &T,
													glm::vec3 &B, const glm::mat3 &invTransform)
{
	...
	if (Material::hasFlag(flags, HasSmoothNormals))
		iN = normalize(w * normal0 + u * normal1 + v * normal2);
	...
	// Texturing
	float tu = 0, tv = 0;
	if (Material::hasFlag(flags, HasDiffuseMap) || Material::hasFlag(flags, HasNormalMap) ||
		Material::hasFlag(flags, HasSpecularityMap) || Material::hasFlag(flags, HasRoughnessMap) ||
		Material::hasFlag(flags, Has2ndDiffuseMap) || Material::hasFlag(flags, Has3rdDiffuseMap) ||
		Material::hasFlag(flags, Has2ndNormalMap) || Material::hasFlag(flags, Has3rdNormalMap))
	{
		tu = w * texu4.x + u * texu4.y + v * texu4.z;
		tv = w * texv4.x + u * texv4.y + v * texv4.z;
	}
	...
	if (Material::hasFlag(flags, HasDiffuseMap))
	{
		...
		const vec4 texel = FetchTexelTrilinear(lambda, uvscale * (uvoffs + glm::vec2(tu, tv)), mat.texaddr0,
											   static_cast<int>(mat.texwidth0), static_cast<int>(mat.texheight0));
		...
		if (Material::hasFlag(flags, Has2ndDiffuseMap)) // must have base texture; second and third layers are additive
		{
			...
			returnValue.color +=
				vec3(FetchTexelTrilinear(lambda, uvscale * (uvoffs + glm::vec2(tu, tv)), mat.texaddr1,
										 static_cast<int>(mat.texwidth1), static_cast<int>(mat.texheight1)));
		}
		if (Material::hasFlag(flags, Has3rdDiffuseMap))
		{
			...
			returnValue.color +=
				vec3(FetchTexelTrilinear(lambda, uvscale * (uvoffs + glm::vec2(tu, tv)), mat.texaddr2,
										 static_cast<int>(mat.texwidth2), static_cast<int>(mat.texheight2)));
		}
		...
		if (Material::hasFlag(flags, HasNormalMap))
		{
			...
			vec3 shadingNormal = (vec3(FetchTexel(uvscale * (uvoffs + vec2(tu, tv)), mat.nmapaddr0, mat.nmapwidth0,
												  mat.nmapheight0, RGBA32)) -
								  vec3(0.5f)) *
								 2.0f;
			if (Material::hasFlag(flags, Has2ndNormalMap))
			{
				...
				vec3 normalLayer1 = (vec3(FetchTexel(uvscale * (uvoffs + vec2(tu, tv)), mat.nmapaddr1, mat.nmapwidth1,
													 mat.nmapheight1, RGBA32)) -
									 vec3(0.5f)) *
									2.0f;
			...
			if (Material::hasFlag(flags, Has3rdNormalMap))
			{
				uvscale = glm::vec2(static_cast<float>(mat.nuscale1), static_cast<float>(mat.nvscale1));
				uvoffs = glm::vec2(static_cast<float>(mat.nuoffs1), static_cast<float>(mat.nvoffs1));
				vec3 normalLayer2 = (vec3(FetchTexel(uvscale * (uvoffs + vec2(tu, tv)), mat.nmapaddr1, mat.nmapwidth1,
													 mat.nmapheight1, RGBA32)) -
									 vec3(0.5f)) *
									2.0f;
