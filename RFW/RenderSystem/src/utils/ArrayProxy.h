...
template <typename T> class ArrayProxy
{
  public:
	...
	template <size_t N> ArrayProxy(std::array<typename std::remove_const<T>::type, N> const &data) : m_Size(N), m_Data(data.data()) {}

	template <class Allocator = std::allocator<typename std::remove_const<T>::type>>
	ArrayProxy(std::vector<typename std::remove_const<T>::type, Allocator> &data) : m_Size(static_cast<uint32_t>(data.size())), m_Data(data.data())
	{
	}

	template <class Allocator = std::allocator<typename std::remove_const<T>::type>>
	ArrayProxy(std::vector<typename std::remove_const<T>::type, Allocator> const &data) : m_Size(static_cast<uint32_t>(data.size())), m_Data(data.data())
	{
	}

	ArrayProxy(std::initializer_list<T> const &data) : m_Size(static_cast<uint32_t>(data.end() - data.begin())) { m_Data = data.begin(); }
