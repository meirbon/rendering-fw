...
// source file information
    ...
    static cl_int getPlatformID(cl_platform_id *platform) {
        ...
        const std::array<std::string, 3> deviceOrder = {"INTEL", "AMD", "NVIDIA"};
        ...
        for (const auto dev_type : deviceTypes) {
            for (const auto &preferred_device : deviceOrder) {
                for (cl_uint i = 0; i < num_platforms; ++i) {
                    ...
                    if (!strstr(buffer, preferred_device.data()))
                        continue;

                    rfw::utils::logger::log("Replaced OpenCL device with %s", buffer);
    ...
    static std::vector<char> loadSource(const char *file, size_t *size) {
        std::string source;
        ...
        *marker = 0;
        ...
        while (!feof(f)) {
            ...
            if (inc) {
                ...
                std::vector<char> incText = loadSource(file_path, size);
        ...
        *size = strlen(source.c_str());
        ...
        std::vector<char> return_value(*size + 1);
        strcpy(return_value.data(), source.c_str());
        return return_value;
    }
    ...
    bool CLContext::init_cl() {
        ...
        for (uint i = deviceUsed; (!canShare && (i < endDev)); ++i) {
            ...
            if (extensionSize > 0) {
                ...
                CheckCL(error = clGetDeviceInfo(devices[i], CL_DEVICE_EXTENSIONS, extensionSize,
                                                extension_string.data(), &extensionSize));
                std::string device_extensions(extension_string.data());
                std::transform(device_extensions.begin(), device_extensions.end(), device_extensions.begin(),
                               ::tolower);
                size_t oldPos = 0;
        ...
        const cl_context_properties props[] = {CL_CONTEXT_PROPERTY_USE_CGL_SHAREGROUP_APPLE, (cl_context_properties)kCGLShareGroup, CL_CONTEXT_PLATFORM,
    ...
    void CLContext::submit(CLKernel &kernel) const {
        CheckCL(clEnqueueNDRangeKernel(m_Queue, kernel.get_kernel(), kernel.get_dimensions(), 0, kernel.get_work_size(),
    ...
    CLKernel::CLKernel(std::shared_ptr<CLContext> context, const char *file, const char *entryPoint,
                       std::array<size_t, 3> workSize,
                       std::array<size_t, 3> localSize)
            : m_Context(context), m_WorkSize(workSize), m_LocalSize(localSize) {
        size_t size;
        cl_int error;
        std::vector<char> source = loadSource(file, &size);
        const char* source_ptr = source.data();
        m_Program = clCreateProgramWithSource(context->get_context(), 1, static_cast<const char **>(&source_ptr), &size, &error);
        CheckCL(error);
        ...
        if (error != CL_SUCCESS) {
            ...
            clGetProgramBuildInfo(m_Program, context->get_device(), CL_PROGRAM_BUILD_LOG, log.size(), log.data(),
                                  nullptr);
            FAILURE(log.data());
        }
