...
void rfw::TopLevelBVH::construct_bvh()
{
	m_Nodes.clear();
	AABB rootBounds = {};
...
std::optional<const rfw::Triangle> rfw::TopLevelBVH::intersect(cpurt::Ray &ray, float t_min, int &instID) const
{
	return intersect(ray.origin, ray.direction, &ray.t, &ray.primIdx, t_min, instID);
}

std::optional<const rfw::Triangle> rfw::TopLevelBVH::intersect(const vec3 &org, const vec3 &dir, float *t, int *primID, float t_min, int &instID) const
{
	const auto mask = _mm_set_epi32(0, ~0, ~0, ~0);

	simd::vector4 origin = vec4(org, 1);
	simd::vector4 direction = vec4(dir, 0);

#if USE_TOP_MBVH
	if (!MBVHNode::traverse_mbvh(org, dir, t_min, t, &instID, m_MNodes.data(), m_PrimIndices.data(),
#else
	if (!BVHNode::traverse_bvh(org, dir, t_min, t, &instID, m_Nodes.data(), m_PrimIndices.data(),
#endif
								 [this, &origin, &direction, &mask, &t_min, &t, &primID, &instID](int ID) {
									 const glm_vec4 origin4 = glm_mat4_mul_vec4(inverseMatrices[ID].cols, origin.vec_4);
									 const glm_vec4 direction4 = glm_mat4_mul_vec4(inverseMatrices[ID].cols, direction.vec_4);

									 vec3 tmp_org, tmp_dir;

									 _mm_maskstore_ps(value_ptr(tmp_org), mask, origin4);
									 _mm_maskstore_ps(value_ptr(tmp_dir), mask, direction4);

									 const CPUMesh *mesh = accelerationStructures[ID];
#if USE_MBVH
									 return mesh->mbvh->traverse(tmp_org, tmp_dir, t_min, t, primID);
#else
								   return mesh->bvh->traverse(tmp_org, tmp_dir, t_min, t, primID);
#endif
								 }))
	{
		return std::nullopt;
...
bool rfw::TopLevelBVH::occluded(const vec3 &org, const vec3 &dir, float t_min, float t_max) const
{
	const auto mask = _mm_set_epi32(0, ~0, ~0, ~0);
...
#if USE_TOP_MBVH
	return MBVHNode::traverse_mbvh_shadow(org, dir, t_min, t_max, m_MNodes.data(), m_PrimIndices.data(),
#else
	return BVHNode::traverse_bvh_shadow(org, dir, t_min, t_max, m_Nodes.data(), m_PrimIndices.data(),
#endif
										  [this, &origin, &direction, &mask, &t_min, &t_max](int ID) {
											  const glm_vec4 origin4 = glm_mat4_mul_vec4(inverseMatrices[ID].cols, origin.vec_4);
											  const glm_vec4 direction4 = glm_mat4_mul_vec4(inverseMatrices[ID].cols, direction.vec_4);
											  ...
											  const CPUMesh *mesh = accelerationStructures[ID];
#if USE_MBVH
											  return mesh->mbvh->traverse_shadow(tmp_org, tmp_dir, t_min, t_max);
#else
											return mesh->bvh->traverse_shadow(tmp_org, tmp_dir, t_min, t_max);
#endif
										  });
}
...
void rfw::TopLevelBVH::intersect(cpurt::RayPacket4 &packet, float t_min) const
{
	...
	while (stackPtr >= 0)
	{
		...
		for (int i = 3; i >= 0; i--)
		{
			const int idx = (hit.tmini[i] & 0b11); // reversed order, we want to check best nodes first
			if (hit.result[idx] == 1)
			{
				stackPtr++;
...
void rfw::TopLevelBVH::set_instance(int idx, glm::mat4 transform, CPUMesh *tree, AABB boundingBox)
{
...
AABB rfw::TopLevelBVH::calculate_world_bounds(const AABB &originalBounds, const simd::matrix4 &matrix)
{
