...
cpurt::Ray::CameraParams::CameraParams(const rfw::CameraView &view, uint samples, float epsilon, uint width,
									   uint height)
{
...
cpurt::Ray cpurt::Ray::generateFromView(const cpurt::Ray::CameraParams &camera, int x, int y, float r0, float r1,
										float r2, float r3)
{
	cpurt::Ray ray;
...
cpurt::RayPacket4 cpurt::Ray::generateRay4(const CameraParams &camera, const int x[4], const int y[4],
										   rfw::utils::RandomGenerator *rng)
{
	using namespace rfw::simd;
	cpurt::RayPacket4 query = {};

	for (int i = 0; i < 4; i++)
		query.pixelID[i] = camera.scrwidth * y[i] + x[i];

	static const vector4 one4 = rfw::simd::ONE4;

	const vector4 r04 = _mm_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand());
	const vector4 r14 = _mm_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand());
	vector4 r24 = _mm_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand());
	vector4 r34 = _mm_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand());

	const vector4 blade4 = r04 * vector4(9.0f);

	r24 = r24 - (blade4 * vector4(1.0f / 9.0f)) * vector4(9.0f);

	// const float blade = float(int(r0 * 9));
	// r2 = (r2 - blade * (1.0f / 9.0f)) * 9.0f;
	// float x1, y1, x2, y2;
	static const vector4 piOver4point5_4 = vector4(3.14159265359f / 4.5f);
	// float bladeParam = blade * piOver4point5;
	const vector4 bladeParam14 = blade4 * piOver4point5_4;
	...
	// bladeParam = (blade + 1.0f) * piOver4point5;
	const vector4 bladeParam24 = (blade4 + one4) * piOver4point5_4;
	// x2 = cos(bladeParam);
	// y2 = sin(bladeParam);
	vector4 x24, y24;
	...
	// if ((r2 + r3) > 1.0f)
	const vector4 mask = (r24 + r34) > one4;
	// r2 = 1.0f - r2;
	// r3 = 1.0f - r3;
	_mm_maskstore_ps(reinterpret_cast<float *>(&r24), _mm_castps_si128(mask.vec_4), (one4 - r24).vec_4);
	_mm_maskstore_ps(reinterpret_cast<float *>(&r34), _mm_castps_si128(mask.vec_4), (one4 - r34).vec_4);

	// const float xr = x1 * r2 + x2 * r3;
	// const float yr = y1 * r2 + y2 * r3;
	const vector4 xr4 = (x14.vec_4 * r24) + (x24 * r34);
	const vector4 yr4 = (y14.vec_4 * r24) + (y24 * r34);

	// ray.origin = vec3(camera.pos_lensSize) + camera.pos_lensSize.w * (right * xr + up * yr);
	const vector4 lens_size4 = camera.pos_lensSize.w;
	const vector4 org_x4 = camera.pos_lensSize.x + lens_size4 * (camera.right_spreadAngle.x * xr4 + camera.up.x * yr4);
	const vector4 org_y4 = camera.pos_lensSize.y + lens_size4 * (camera.right_spreadAngle.y * xr4 + camera.up.y * yr4);
	const vector4 org_z4 = camera.pos_lensSize.z + lens_size4 * (camera.right_spreadAngle.z * xr4 + camera.up.z * yr4);

	// const float u = (float(x) + r0) * (1.0f / float(camera.scrwidth));
	// const float v = (float(y) + r1) * (1.0f / float(camera.scrheight));
	vector4 u4 = vector4(float(x[0]), float(x[1]), float(x[2]), float(x[3]));
	vector4 v4 = vector4(float(y[0]), float(y[1]), float(y[2]), float(y[3]));

	u4 = u4 + r04;
	v4 = v4 + r14;

	const vector4 scrwidth4 = vector4(1.0f / float(camera.scrwidth));
	const vector4 scrheight4 = vector4(1.0f / float(camera.scrheight));

	u4 = u4 * scrwidth4;
	v4 = v4 * scrheight4;

	const vector4 p1_x4 = camera.p1.x;
	const vector4 p1_y4 = camera.p1.y;
	const vector4 p1_z4 = camera.p1.z;

	const vector4 right_x4 = camera.right_spreadAngle.x;
	const vector4 right_y4 = camera.right_spreadAngle.y;
	const vector4 right_z4 = camera.right_spreadAngle.z;

	const vector4 up_x4 = camera.up.x;
	const vector4 up_y4 = camera.up.y;
	const vector4 up_z4 = camera.up.z;

	// const vec3 pointOnPixel = p1 + u * right + v * up;
	const vector4 pixel_x4 = p1_x4 + (u4 * right_x4) + (v4 * up_x4);
	const vector4 pixel_y4 = p1_y4 + (u4 * right_y4) + (v4 * up_y4);
	const vector4 pixel_z4 = p1_z4 + (u4 * right_z4) + (v4 * up_z4);

	// ray.direction = pointOnPixel - ray.origin;
	vector4 dir_x4 = (pixel_x4 - org_x4);
	vector4 dir_y4 = (pixel_y4 - org_y4);
	vector4 dir_z4 = (pixel_z4 - org_z4);

	// ray.direction = normalize(ray.direction);
	vector4 length_squared_4 = dir_x4 * dir_x4 + dir_y4 * dir_y4 + dir_z4 * dir_z4;
	const vector4 inv_length = length_squared_4.inv_sqrt();
	dir_x4 = dir_x4 * inv_length;
	dir_y4 = dir_y4 * inv_length;
	dir_z4 = dir_z4 * inv_length;
...
cpurt::RayPacket8 cpurt::Ray::generateRay8(const CameraParams &camera, const int x[8], const int y[8],
										   rfw::utils::RandomGenerator *rng)
{
	...
	r04 = _mm256_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(),
						rng->Rand());
	r14 = _mm256_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(),
						rng->Rand());
	r24 = _mm256_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(),
						rng->Rand());
	r34 = _mm256_set_ps(rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(), rng->Rand(),
						rng->Rand());
