...
void vkrtx::Context::render_frame(const rfw::Camera &cam, rfw::RenderStatus status)
{
	...
	for (uint i = 0; i < m_SamplesPP; i++)
	{
		...
		// Primary ray stage
		if (m_SamplesTaken <= 1)
			cmdBuffer->fillBuffer(static_cast<vk::Buffer>(*m_AccumulationBuffer), 0,
								  m_ScrWidth * m_ScrHeight * sizeof(glm::vec4), 0);
	...
	m_UniformFinalizeParams->get_data()[0] =
...
void vkrtx::Context::set_lights(rfw::LightCount lightCount, const rfw::DeviceAreaLight *areaLights,
