...
int intersect4(cpurt::RayPacket4 &packet, const glm::vec4 &p0, const glm::vec4 &p1, const glm::vec4 &p2, __m128 *store_mask, float epsilon)
{
	...
	const __m128 edge1_x = _mm_set1_ps(edge1[0]);
	const __m128 edge1_y = _mm_set1_ps(edge1[1]);
	const __m128 edge1_z = _mm_set1_ps(edge1[2]);

	const __m128 edge2_x = _mm_set1_ps(edge2[0]);
	const __m128 edge2_y = _mm_set1_ps(edge2[1]);
	const __m128 edge2_z = _mm_set1_ps(edge2[2]);
...
bvh::AABB getBounds(const glm::vec3 &p0, const glm::vec3 &p1, const glm::vec3 &p2)
{
...
bvh::AABB getBounds(const glm::vec4 &p04, const glm::vec4 &p14, const glm::vec4 &p24)
{
	const vec3 p0 = vec3(p04);
	const vec3 p1 = vec3(p14);
	const vec3 p2 = vec3(p24);
