...
void VkContext::createSurface()
{
	...
	if (glfwCreateWindowSurface((VkInstance)m_Instance, m_Window->getGLFW(), nullptr,
...
void VkContext::createGraphicsPipeline()
{
	...
	m_GraphicsPipeline = m_Device->createGraphicsPipeline(nullptr, pipelineInfo, nullptr).value;
...
{
	const auto buffers = m_SwapChain->getBuffers();
	m_CommandBuffers = m_Device.createCommandBuffers(uint32_t(buffers.size()), vk::CommandBufferLevel::ePrimary,
													 VulkanDevice::GRAPHICS);
	...
	for (size_t i = 0; i < buffers.size(); i++)
	{
		...
		vk::ImageMemoryBarrier imageBarrier =
			vk::ImageMemoryBarrier({}, {}, vk::ImageLayout::eGeneral, vk::ImageLayout::eColorAttachmentOptimal,
								   m_Device.getQueueIndices().graphicsIdx.value(),
...
void vkc::VkContext::set_materials(const std::vector<rfw::DeviceMaterial> &materials,
...
void vkc::VkContext::set_lights(rfw::LightCount lightCount, const rfw::DeviceAreaLight *areaLights,
