...
class MBVHNode
{
  public:
	...
	void merge_nodes(const BVHNode &node, const rfw::utils::array_proxy<BVHNode> bvhPool, MBVHNode *bvhTree,
	...
	void validate(rfw::utils::array_proxy<MBVHNode> nodes, rfw::utils::array_proxy<uint> primIDs, uint maxPoolPtr,
				  uint maxPrimIndex) const;

	void merge_node(const BVHNode &node, rfw::utils::array_proxy<BVHNode> pool, int &numChildren);
};
} // namespace bvh
} // namespace rfw