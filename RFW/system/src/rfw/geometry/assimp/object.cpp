...
assimp::Object::Object(std::string_view filename, material_list *matList, uint ID, const mat4 &matrix, int material)
{
	...
	auto meshes = std::vector<std::vector<TmpPrim>>(scene->mNumMeshes);
	...
	for (const auto &node : sceneNodes)
	{
		if (animNodeMeshes.find(node.name) != animNodeMeshes.end())
			object.nodes.push_back(
				SceneNode(&object, node.name, node.childIndices, node.meshIDs, {0}, meshes, {}, node.transform));
		else
			object.nodes.push_back(
				SceneNode(&object, node.name, node.childIndices, node.meshIDs, {}, meshes, {}, node.transform));
	}
...
assimp::Object::Object(std::string_view filename, material_list *matList, uint ID, const mat4 &matrix, bool normalize,
					   int material)
	: m_File(filename.data()), m_ID(ID)
{
	...
	for (auto &tri : m_Triangles)
	{
		...
		if (texID > -1)
		{
			const texture &texture = matList->get_textures().at(texID);
...
void assimp::Object::set_time(const float timeInSeconds)
{
	const simd::vector4 normal_mask = _mm_set_epi32(0, ~0, ~0, ~0);
...
void assimp::Object::updateTriangles()
{
...
void assimp::Object::updateTriangles(const std::vector<glm::vec2> &uvs)
{
	m_Triangles.resize(m_Indices.size());
...
std::vector<bool> assimp::Object::get_changed_matrices()
{
...
void assimp::Object::prepare_meshes(system &rs)
{
...
glm::mat4 assimp::Object::AnimationChannel::getInterpolatedTRS(float time) const
{
...
void assimp::Object::AssimpNode::update(std::vector<AssimpNode> &nodes, const glm::mat4 &T)
{
