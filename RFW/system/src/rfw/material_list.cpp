...
material_list::material_list() { add(HostMaterial()); }
...
uint material_list::add(const HostMaterial &mat)
{
...
uint material_list::add(const aiMaterial *aiMat, const std::string_view &basedir)
{
	HostMaterial mat{};

	aiColor3D ambient = {0, 0, 0}, diffuse = {0, 0, 0}, specular = {0, 0, 0}, emissive = {0, 0, 0},
			  transparent = {0, 0, 0};
	...
	for (uint i = 0; i < aiMat->GetTextureCount(aiTextureType_DIFFUSE) && i < 3; i++)
	{
		...
		const int idx = get_texture_index(file);
	...
	for (uint i = 0; i < aiMat->GetTextureCount(aiTextureType_NORMALS) && i < 3; i++)
	{
		...
		const uint idx = get_texture_index(file);
		switch (i)
		{
		case (0):
	...
	if (aiMat->GetTextureCount(aiTextureType_SPECULAR) > 0)
	{
		...
		mat.map[SPECULARITY].textureID = get_texture_index(file);
	}
	...
	if (aiMat->GetTextureCount(aiTextureType_SHININESS) > 0)
	{
		...
		mat.map[ROUGHNESS0].textureID = get_texture_index(file);
	}
	...
	if (aiMat->GetTextureCount(aiTextureType_OPACITY) > 0 && mat.map[TEXTURE0].textureID != -1)
	{
		...
		const int textureIndex = get_texture_index(file);
...
uint rfw::material_list::add(const texture &tex)
{
	const uint idx = static_cast<uint>(m_Textures.size());
...
uint rfw::material_list::add(texture &&tex)
{
	const uint idx = static_cast<uint>(m_Textures.size());
...
void rfw::material_list::set(uint index, const HostMaterial &mat)
{
	bool valid = true;
...
void material_list::generate_device_materials()
{
...
bool material_list::is_dirty() const { return m_IsDirty; }

int material_list::get_texture_index(const std::string_view &file)
{
...
std::pair<Material, MaterialTexIds> HostMaterial::convertToDeviceMaterial(material_list *list) const
{
	Material gpuMat{};
	...
	const auto &textures = list->get_textures();
