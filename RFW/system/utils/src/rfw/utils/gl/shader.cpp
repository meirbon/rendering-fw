...
shader::shader(std::string vertexPath, std::string fragmentPath)
	: m_VertPath(std::move(vertexPath)), m_FragPath(std::move(fragmentPath))
{
	m_ShaderId = load();
}
...
GLuint shader::load()
{
	...
	check_compile_errors(m_VertPath.data(), vert_shader, "VERTEX");

	glShaderSource(frag_shader, 1, &frag_source, nullptr);
	glCompileShader(frag_shader);
	check_compile_errors(m_FragPath.data(), frag_shader, "FRAGMENT");
	...
	check_compile_errors(m_VertPath.data(), program, "PROGRAM");
...
void shader::check_compile_errors(const char *file, GLuint shader, const std::string &type) const
{
	GLint success;
	...
	if (type != "PROGRAM")
	{
		...
		if (!success)
		{
			...
			FAILURE("shader compilation error of type: %s\n%s", type.c_str(), infoLog.data());
		}
	}
	else
	{
