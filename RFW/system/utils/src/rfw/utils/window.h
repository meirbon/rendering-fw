...
{
  public:
	window(int width, int height, const char *title, bool resizable = false, bool hidpi = false,
		   std::optional<std::pair<unsigned int, unsigned int>> glVersion = std::nullopt,
		   std::optional<unsigned int> msaa = std::nullopt)
	{
		m_Flags[HIDPI] = hidpi;
		...
		glfwSetKeyCallback(m_Instance, window::key_callback);
		glfwSetCursorPosCallback(m_Instance, window::cursor_position_callback);
		glfwSetMouseButtonCallback(m_Instance, window::mouse_button_callback);
		glfwSetScrollCallback(m_Instance, window::mouse_scroll_callback);
		glfwSetWindowSizeCallback(m_Instance, window::resize_callback);
	}
	...
	bool should_close() { return glfwWindowShouldClose(m_Instance); }
	void poll_events()
	{
		glfwPollEvents();
	...
	[[nodiscard]] bool pressed(key_code key) const
	{
...
#endif
		return static_cast<int>(static_cast<float>(get_framebuffer_width()) / get_render_width_scale());
	}
	...
	[[nodiscard]] int get_render_height() const
	{
		...
		return getFramebufferWidth();
#endif
		return static_cast<int>(static_cast<float>(get_framebuffer_height()) / get_render_height_scale());
	}
	...
	[[nodiscard]] int get_framebuffer_width() const
	{
		int width, height;
	...
	[[nodiscard]] int get_framebuffer_height() const
	{
		int width, height;
		glfwGetFramebufferSize(m_Instance, &width, &height);
		return height;
	...
	inline void add_mouse_pos_callback(std::function<void(double, double, double, double)> callback)
	{
		m_PosCallbacks.emplace_back(callback);
	}
	inline void add_keys_callback(std::function<void(const std::vector<bool> &, const std::vector<bool> &)> callback)
	{
		m_KeysCallbacks.emplace_back(callback);
	}
	inline void add_scroll_callback(std::function<void(double, double)> callback)
	{
		m_ScrollCallbacks.emplace_back(callback);
	}
	inline void add_resize_callback(std::function<void(int, int)> callback)
	{
		m_ResizeCallbacks.emplace_back(callback);
	}

	std::vector<std::function<void(int, int)>> &get_resize_callbacks() { return m_ResizeCallbacks; }
	std::vector<std::function<void(double, double, double, double)>> &get_mouse_pos_callbacks()
	{
		return m_PosCallbacks;
	}
	std::vector<std::function<void(const std::vector<bool> &, const std::vector<bool> &)>> &get_keys_callbacks()
	{
  ...
  private:
	...
	static void cursor_position_callback(GLFWwindow *w, double xpos, double ypos)
	{
		...
		const double x = xpos / static_cast<double>(width);
		const double y = ypos / static_cast<double>(height);
