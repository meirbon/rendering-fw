...
void main()
{
    ...
    if (!any(greaterThan(color, vec3(1))))
    {
        ...
        for (int i = 0; i < SPOT_LIGHT_COUNT; i++)
        {
            ...
            if (NdotL > 0 && LNdotL > 0)
            {
                finalColor += evalLighting(color, roughness, normal, T, B, forward, L) * spotLights[i].radiance_cos_outer.xyz * NdotL * LNdotL * (1.0f / dist2);
            }
        }
        ...
        for (int i = 0; i < DIR_LIGHT_COUNT; i++)
        {
            ...
            vec3 L = -light.direction_energy.xyz;
            if (NdotL > 0)
            {
