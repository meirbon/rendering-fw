...
QueueFamilyIndices::QueueFamilyIndices(vk::PhysicalDevice device, std::optional<vk::SurfaceKHR> surface) : hasSurface(surface.has_value())
{
	...
	for (const auto &qf : queueFamilies)
	{
		if (qf.queueCount > 0)
		{
			...
			if (surface && !this->presentIdx.has_value())
			{
				if (device.getSurfaceSupportKHR(i, surface.value()))
...
{
	const auto index = m_Members->queueCmdPoolIndices[type];
	const auto cmdPool = m_Members->m_CommandPools.at(index);

	m_Members->m_VkDevice.freeCommandBuffers(cmdPool, {cmdBuffer});
}
...
void VulkanDevice::freeCommandBuffers(const std::vector<vk::CommandBuffer> &cmdBuffers, QueueType type)
{
	...
	m_Members->m_VkDevice.freeCommandBuffers(cmdPool, cmdBuffers);
}
...
unsigned int VulkanDevice::rateDevice(const vk::PhysicalDevice &pDevice, std::optional<vk::SurfaceKHR> surface)
{
