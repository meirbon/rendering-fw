	...
	: m_Device(dev), m_Extent(extent)
{
	...
	if (allocType == TEXTURE)
	{
		allocInfo.preferredFlags = (VkMemoryPropertyFlags)memProps;
	}
	else if (allocType == FRAMEBUFFER || allocType == SKYDOME)
	{
		allocInfo.requiredFlags = (VkMemoryPropertyFlags)memProps;
	...
	if (!m_Image)
		FAILURE("Could not create image.");
	...
	//
	// if (!m_Memory)
	//	FAILURE("Could not allocate memory for image.");
	//
...
bool Image::setData(const void *data, uint32_t width, uint32_t height, uint32_t stride)
{
...
bool Image::createImageView(vk::ImageViewType viewType, vk::Format format, vk::ImageSubresourceRange subresourceRange)
{
	...
	if (!m_ImageView)
		FAILURE("Could not create image view.");
	return true;
}
						  ...
						  vk::SamplerAddressMode addressMode)
{
	...
	if (!m_Sampler)
		FAILURE("Could not create sampler.");

	return true;
}

void Image::transitionToLayout(vk::ImageLayout dstLayout, vk::AccessFlags dstAccessMask, vk::CommandBuffer cmdBuffer)
{
	vk::ImageMemoryBarrier barrier{};
	barrier.setPNext(nullptr);
	barrier.setSrcAccessMask(vk::AccessFlags());
