...
#ifndef RENDERING_FW_VKCONTEXT_SRC_CHECKVK_HPP
#define RENDERING_FW_VKCONTEXT_SRC_CHECKVK_HPP
...
static void _CheckVK(int line, const char *file, vk::Result x)
{
	...
	char errBuffer[512];
	sprintf(errBuffer, "Vulkan error on line %i in %s: %i = %s", line, file, int(x), error);
	throw std::runtime_error(errBuffer);
}

#endif // RENDERING_FW_VKCONTEXT_SRC_CHECKVK_HPP
